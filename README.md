...
As also explained in that other project, the namespace of the control needs to be defined. In contrast to there, now an `@name` JSDoc tag with the *full* name is used (but both ways are valid!):

```
@name com.myorg.myui5lib.Example
```

Also in contrast to the custom control in that other project, the Renderer is implemented in a separate file, like it is typically done in the original UI5 libraries. Again, both options are equally valid! 
